...
![标志寄存器位分布](Assembly_language.assets/IMG_0274.PNG)

这节课中，我们学习标志位寄存器中的 CF, PF, ZF, SF, OF, DF 标志位

### 11.1 ZF标志（Zoro flag）

flag中的第6位是`ZF`位，零标志位。**它记录相关指令执行后，其结果是否为0**。
...
注意，在8086CPU 的指令集中，有的指令的执行是影响标志寄存器的，比如，`add`、`sub`、`mul`、 `div`、`inc`、 `or`、 `and` 等，它们大都是运算指令(进行逻辑或算术运算)；有的指令的执行对标志寄存器没有影响，比如，`mov`、`push`、`pop` 等，它们大都是传送指令。在使用一条指令的时候，要注意这条指令的全部功能，其中包括，执行结果对标志寄存器的哪些标志位造成影响

### 11.2 PF标志（Parity flag）

flag的第2位是`PF`，奇偶标志位。**它记录相关指令执行后，其结果的所有位中的1的个数是否为偶数**。如果1的个数为偶数，`pf=1`，如果为奇数，`pf=0`。
...
flag的第7位是`SF`标志位，符号标志位。**它记录相关的指令执行后，其结果是否为负**。如果结果为负，`fs=1`，如果是非负，`fs=0`。

在计算机中，对于同一个二进制数据，计算机可以将它当作无符号数据来运算，也可以当作有符号数据来运算。
...
可以将 `add` 指令进行的运算当作无符号数的运算，那么`add` 指令相当于计算 `129+1`，结果为 `130(10000010B)`：也可以将add 指令进行的运算当作有符号数的运算，那么`add` 指令相当于计算`-127+1`，结果为`-126(10000010B）`。

SF标志就是对有符号数运算结果的一种记录，它记录数据的正负。在我们将数据当作有符号数来运算时，可以通过它来得知结果的正负。如果我们将数据当作无符号数来运算，`sf`的值则没有意义，虽然相关的指令影响了它的值。

### 11.4 CF标志（Carry flag）

flag的第0位是CF标志位，进位标志位。一般情况下，**在进行`无符号数`运算时，它记录了运算结果的`最高`有效位向更高位的进位值，或从最高位的进位值**。

我们知道，当两个数据相加的时候，有可能产生从最高有效位向更高位的进位。比如，两个8位数据：`98H+98H`，将产生进位。由于这个进位值在8位数中无法保存，我们在前面的课程中，就只是简单地说这个进位值丢失了。其实 CPU 在运算的时候，并不丢弃这个进位值，而是记录在一个特殊的寄存器的某一位 上。8086CPU 就用flag 的 CF 位来记录这个进位值。
...
而当两个数据做减法的时候，有可能向更高位借位。比如，两个8位数据：`97H-98H`，将产生借位，借位后，相当于计算 `197H-98H`。而flag 的 `CF` 位也可以用来记录这个借位值。

比如，下面的指令：

```asm
mov al, 97H 
sub al, 98H   ;执行后：(a1)=FFH， CF=1，CE 记录了向更高位的借位值
sub al, al    ;执行后：(a1)=0，CF=0，CF 记录了向更高位的借位值
```

### 11.5 OF标志（Overflow flag）

flag的第11位是1，溢出标志位。一般情况下，**OF记录了`有符号数`运算符的结果是否发生了溢出**。如果发生了溢出，`OF=1`，如果没有，`OF=0`。

一定要注意 `CF` 和`OF` 的区别：`CF` 是对**无符号数**运算有意义的标志位，而`OF` 是对**有符号数**运算有意义的标志位。比如：
...
`CF=0, OF=1`。前面我们讲过，CPU 在执行 add 等指令的时候，就包含了两种含义：无符号数运算和有符号数运算。对于**无符号数**运算，CPU 用`CF` 位来记录是否产生了进位；对于**有符号数**运算，CPU 用 `OF` 位来记录是否产生了滥出，当然，还要用 `SF` 位来记录结果的符号。对于**无符号数**运算，`98+99` 没有进位，`CF=0`；对于有符号数运算，`98+99` 发生溢出，`OF=1`。
...
![进制](./Assembly_language.assets/8FF960A676B5CD31F1228E97E08996E7.png)

对于负数 ff的十进制为-1，加5后等于4

这里补码的运算发生了进位，而在结果中，舍弃进位位，使得补码转盘转回正数的起址，使得运算正常没有发生溢出

在这之前有个误区，认为补码值越高，负数值越大，但事实上却恰恰相反，最大的补码值对应最小的负数，使得负数加正数是如此的自然

在这里不得感叹补码的发明是计算机最伟大的发明



对OF标志位的一些快速解决的方法。

正数加正数，负数加负数可能会发生溢出，可能发生进位，进位就看最高位是否进位，溢出就看次高位是否发生进位

而正数加负数不会发生溢出，可能发生进位



对于有符号数，区分正数和负数在于最高位的值是否为1，在16进制中，最高位是否大于8



### 11.6 adc指令

ade 是带进位加法指令，**它利用了 CF 位上记录的进位值**。
指令格式：adc 操作对象1，操作对象2
功能：操作对象1=操作对象1＋ 操作对象2+ CF
比如指令adc ax,bx 实现的功能是：(ax)=(ax)+(6x)+ CF
...
add al,al 
adc al, 3
; 执行后，(al)-34H。adc 执行时，相当于计算：(a1)+3+CF=301+3+1=34H。
; 可以看出，adc 指令比add 指令多加了一个CF 位的值。
```

通过adc指令，可以实现超过16位数据的加法运算。比如：
...
这两个数均为64位数，在8086cpu上无法直接计算。可以使用低16位先相加的方法，如果有进位，其`CF`会设置为1。

而在高16位运算时，再加上`CF`位就可以实现大数的计算。

对于更高位数的数据，可以重复多次类似的计算
...
功能：操作对象 1=操作对象1 - 操作对象2-CF
比如指令`sbb ax,bx` 实现的功能是：`(ax)=(ax)-(bx)-CF`
sbb 指令执行后，将对 CF 进行设置。利用sbb 指令可以对任意大的数据进行减法运算。比如，计算 003E1000H-00202000H，结果放在ax,bx 中，程序如下：

```asm
mov bx, 1000H
mov ax, 003EH
sub bx, 2000H sbb ax, 0020H
```

`sbb`和`adc` 是基于同样的思想设计的两条指令，在应用思路上和adc 类似。在这里，我们就不再进行过多的讨论。通过学习这两条指令，我们可以进一步领会一下标志寄存器CF 位的作用和意义

### 11.8 cmp指令

`cmp` 是比较指令，`cmp` 的功能相当于减法指令，只是不保存结果。`cmp` 指令执行后，将对标志寄存器产生影响。其他相关指令通过识别这些被影响的标志寄存器位 来得知比较结果。

`cmp `指令格式：cmp 操作对象1，操作对象2

功能：计算操作对象1-操作对象2，但并不保存结果，仅仅根据计算结果对标志寄存器进行设置。

比如，指令cmp ax,ax，做(ax)-(ax)的运算，结果为 0，但并不在 ax 中保存，仅影响
flag 的相关各位。指令执行后：`zf=1``,pf=1`,`sf=0`, `cf=0`, `of=0`。
下面的指令：
...
mov bx, 3 
cmp ax, bx
; 执行后：(ax)-8,2f-0, pf=1, sf=0, cf=0, of-0。
```

其实，我们通过 cmp 指令执行后，相关标志位的值就可以看出比较的结果。

```asm
指令 cmp ax,bx 的逻辑含义是比较ax 和bx 中的值，如果执行后：
ZF=1，说明(ax)= (bx)
ZF=0，说明(ax)大(bx)
CF=1，说明(ax)≤(bx)
CF=0，说明(ax)≥(6x)
...
同add、sub 指令一样，CPU 在执行 cmp 指令的时候，也包含两种含义：进行无符号数运算和进行有符号数运算。所以利用 cmp 指令可以对无符号数进行比较，也可以对有符号数进行比较
